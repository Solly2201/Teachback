  ...
  return (
    <div className="space-y-5">
      ...
      <p className="text-sm text-charcoal-light">
        ...
        nothing is final until you press <strong>Start TeachBack</strong>.
      </p>

      {/* concepts */}
      <div className="card">
        <div className="card-header">
          <span>Important concepts detected</span>
          <button onClick={() => setDraftList('concepts', (l) => [...l, { name: '', description: '' }])} className="text-white/90 hover:text-white normal-case font-normal">+ add concept</button>
        </div>
          ...
          {(draft.concepts || []).map((c, i) => (
            <div key={i} className="border border-zinc-200 rounded-md p-3 grid md:grid-cols-3 gap-3 relative">
              <div>
                ...
                <input className="input" value={c.name} onChange={(e) => setDraftList('concepts', (l) => l.map((x, j) => (j === i ? { ...x, name: e.target.value } : x)))} />
              </div>
              <div className="md:col-span-2">
                <label className="label">Possible explanation (from the lecture — edit freely)</label>
                <input className="input" value={c.description} onChange={(e) => setDraftList('concepts', (l) => l.map((x, j) => (j === i ? { ...x, description: e.target.value } : x)))} />
              </div>
