          ...
          {topics?.map((t) => (
            <button key={t.id} onClick={() => openTopic(t.id)} className="card p-5 text-left hover:border-brand hover:shadow-md transition-all group">
              ...
              <div className="text-xs text-charcoal-light mt-3">
                {t.concept_count} concepts · {t.misconception_count} misconceptions · {t.activity_count} activities
              </div>
            </button>
          ))}
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="card p-5 space-y-4">
        ...
        <div>
          <label className="label">Reference explanation (model answer used for semantic comparison)</label>
        ...
        <div className="p-4 space-y-3">
          ...
          {editing.concepts.map((c, i) => (
            <div key={i} className="border border-zinc-200 rounded-md p-3 grid md:grid-cols-2 gap-3 relative">
              ...
              <div>
                <label className="label">Description (compared with student sentences)</label>
